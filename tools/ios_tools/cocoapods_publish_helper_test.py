...
class CocoapodsPublishHelperTest(unittest.TestCase):
    ...
    def test_check_version_published_uses_private_specs_repo_layout(self):
        home = Path(self.temp_dir.name) / 'home'
        repo_root = home / '.cocoapods' / 'repos' / 'lynx-specs' / 'Lynx' / '1.2.3'
        repo_root.mkdir(parents=True, exist_ok=True)
        (repo_root / 'Lynx.podspec.json').write_text('{}', encoding='utf8')
        self.write_podspec_json('Lynx', {'version': '1.2.3'})

        with mock.patch.object(helper.Path, 'home', return_value=home):
            self.assertTrue(helper.check_version_published('Lynx', 'lynx-specs'))
    ...
    def test_push_specs_repo_pushes_current_branch_with_auth_env(self):
        with mock.patch.object(helper.subprocess, 'check_output', return_value='main\n'), \
                mock.patch.object(helper, 'run_command') as run_command:
