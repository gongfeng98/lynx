...
def publish_component(component, sources):
    if sources != None:
        run_command(f'bundle exec pod repo push my_specs {component}.podspec.json --verbose --skip-import-validation --allow-warnings --skip-tests --sources={sources}')
    else:
        run_command(f' bundle exec pod repo push my_specs {component}.podspec.json --verbose --skip-import-validation --allow-warnings --skip-tests')
