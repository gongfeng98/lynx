...
def published_spec_path(component, version, repo_name):
    return Path.home() / '.cocoapods' / 'repos' / repo_name / component / version / f'{component}.podspec.json'
...
def check_version_published(component, repo_name):
    version = get_podspec_version(component)
    print(f'Checking if {component} version {version} is already published...')
    if published_spec_path(component, version, repo_name).exists():
...
def publish_component(component, sources, repo_name, repo_path, git_auth_env):
    ...
    for attempt in range(1, PUBLISH_RETRY_ATTEMPTS + 1):
        if check_version_published(component, repo_name):
            return
